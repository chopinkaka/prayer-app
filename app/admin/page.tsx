    ...
    return (
      <div style={s.lockWrap}>
        <div style={s.lockCard}>
          ...
          <h2 style={{ fontSize: 20, fontWeight: 800, color: colors.primaryDark, marginBottom: 6 }}>
            관리자 페이지
          </h2>
    ...
    <div style={s.adminWrap}>
      <div style={s.container}>
        <div style={{ textAlign: 'center', marginBottom: 24 }}>
          <h1 style={{ fontSize: 22, fontWeight: 800, color: colors.primaryDark }}>
            ✝️ 중보기도 기도제목 수합
          </h1>
        ...
        <div style={{ display: 'flex', gap: 12, marginBottom: 20 }}>
          <div style={s.statCard}>
            <div style={{ fontSize: 30, fontWeight: 800, color: colors.primary }}>{data.length}</div>
            <div style={{ fontSize: 12, color: '#aaa' }}>총 기도제목</div>
          </div>
          <div style={s.statCard}>
            <div style={{ fontSize: 30, fontWeight: 800, color: colors.primary }}>{todayCount}</div>
        ...
        {loading ? (
          <div style={{ textAlign: 'center', padding: 48, color: colors.primary }}>🙏 불러오는 중...</div>
        ) : sorted.length === 0 ? (
            ...
            {sorted.map((item) => (
              <div key={item.id} style={s.prayerCard}>
                <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 8 }}>
                  <span style={{ fontWeight: 700, color: colors.primaryDark }}>{item.name}</span>
...
const s = {
  lockWrap: {
    ...
    minHeight: '100vh', padding: 20, background: colors.bg,
  } as React.CSSProperties,
  lockCard: {
    background: 'white', borderRadius: 20, padding: '44px 36px',
    width: '100%', maxWidth: 380, boxShadow: '0 8px 32px rgba(46,125,50,0.13)',
    textAlign: 'center',
  } as React.CSSProperties,
  pwInput: {
    width: '100%', border: `2px solid ${colors.border}`, borderRadius: 10,
  ...
  lockBtn: {
    ...
    background: `linear-gradient(135deg, ${colors.primary}, ${colors.primaryDark})`, color: 'white',
    border: 'none', borderRadius: 12, fontSize: 15, fontWeight: 700, cursor: 'pointer',
  } as React.CSSProperties,
  adminWrap: { background: colors.bg, minHeight: '100vh', padding: '24px 16px 48px' } as React.CSSProperties,
  container: { maxWidth: 700, margin: '0 auto' } as React.CSSProperties,
  statCard: {
    flex: 1, background: 'white', borderRadius: 12, padding: '16px 10px',
    textAlign: 'center', boxShadow: '0 2px 10px rgba(46,125,50,0.07)',
  } as React.CSSProperties,
  prayerCard: {
    background: 'white', borderRadius: 14, padding: '18px 52px 18px 20px',
    boxShadow: '0 2px 12px rgba(46,125,50,0.07)', borderLeft: `4px solid ${colors.primary}`,
    position: 'relative',
  } as React.CSSProperties,
  delBtn: {
  ...
  tbtn: (color: 'purple' | 'gray') => ({
    ...
    background: color === 'purple' ? colors.primary : '#f0f0f0',
    color: color === 'purple' ? 'white' : '#666',
  } as React.CSSProperties),
  sortBtn: (active: boolean) => ({
    padding: '6px 14px', borderRadius: 20, border: `2px solid ${colors.border}`,
